...
> 设计与执行计划见 `doc/rag_data_source_tree_plan_20260909.md`。三大需求：①导入入口统一为「数据源选择」（文件/文件夹只是选项）；②新增 Git 数据源（gix 纯 Rust 集成、支持认证）；③列表增加树形视图（存量归「文件」节点）。版本 `1.0.35002`（后并入 1.0.35003 发布，1.0.35003 的 changelog 又随 2026-09-24 基线同步并入 `upgrade/1.0.40002.md`，原 `1.0.35003.md` 已删）。
...
- 版本：`1.0.35001 → 1.0.35003`（tauri.conf.json / Cargo.toml / 根 package.json / frontend package.json + Cargo.lock）；changelog **与 1.0.35003 合并为单文件 `doc/upgrade/1.0.35003.md`**（原 `1.0.35002.md` 已删；该文件后续随 2026-09-24 基线同步再并入 `upgrade/1.0.40002.md`，`1.0.35003.md` 已删）。
...
**验证**：`cargo test --lib` **41 passed**；`cargo check` 0 错 0 警；`npm run build` ✓；`npx tsc --noEmit` 24 = 基线。版本 `1.0.35002 → 1.0.35003`（四源 + Cargo.lock）；changelog `doc/upgrade/1.0.35003.md`（后续并入 `upgrade/1.0.40002.md`，原文件已删）。
...
| **对应 origin tag**            | `v1.0.40`（最新 tag） |
| **桌面端版本号**               | `1.0.40002` |
| **同步执行日期**               | 2026-09-25            |

> 下次同步时，使用 `85a530f` 作为新的基线 SHA 起点（命令：`cd mcphub-origin && git --no-pager log --oneline 85a530f..HEAD`）。
>
> 注：本节「对应 origin tag」指 origin 仓库的最新 tag（与子模块指针所在 commit 未必相同——指针停在 tag 之后的未发布提交上）。本轮基线仍在 v1.0.40 之内（origin 无新 tag），版本号 `1.0.40001 → 1.0.40002` 序号递增；本轮 6 个 commit（#1210 env 遮蔽警示、#1211 按钮指针光标、#1212 嵌入任务前缀，其余 3 个纯 docs/CI），前端改动集中在 Smart Routing 区块（桌面 Tauri 下隐藏，代码保留对齐）。上一轮（2026-09-24 第二轮）`8ed6478 → f8615ab` 2 个纯 Node 后端修复；第一轮跨 `v1.0.36`~`v1.0.39` 四个 release（`6b1fdb7 → 8ed6478`，43 个 commit），版本号 `1.0.35003 → 1.0.39001`。

### 4.4 最近同步记录

#### 2026-09-25：同步 `f8615ab` -> `85a530f`（6 个 commit，v1.0.40 后未发布提交）

`cd mcphub-origin && git --no-pager log --oneline f8615ab..85a530f` 共 6 个 commit；`git diff --stat f8615ab..85a530f -- frontend/ locales/`：`SettingsContext.tsx`（+18）、`SettingsPage.tsx`（+943/-423）、`index.css`（+7）、4 个 locales（各 +6）。注意：上一轮同步（2026-09-24）后，openapi 功能会话曾把子模块指针无记录推进 `f8615ab → 9ad55da/工作区`，本轮以工作区 `85a530f`（= origin/main）为终点补齐基线。

**已同步到 desktop（前端 / locales）**

| 来源 commit | 说明 | desktop 应用方式 |
| ----------- | ---- | ---------------- |
| `f6e4cc8` #1210 | fix(smart-routing): 环境变量遮蔽仪表盘配置时显式提示（issue #642） | ①`SettingsContext.tsx` `SmartRoutingConfig` 加只读元数据 `envOverriddenFields?: Array<{field; envVar}>`（服务器上报运行时值来自 env 的字段，保存不回传）+ 读取映射 `?? []`；②`SettingsPage.tsx` 新增 `renderEnvOverrideWarning(field)` helper（琥珀色警示行，键 `settings.smartRoutingEnvOverrideWarning`），挂到 openai 分支 3 个输入框（llmProviderApiKey/llmProviderBaseUrl/embeddingModel）+ azure 分支 2 个（azureOpenaiEndpoint/azureOpenaiApiKey）之后；③4 个 locales 各 +1 键（与 origin 值逐语言一致）。 |
| `7b2822d` #1212 | feat(smart-routing): 嵌入查询/文档任务前缀（EmbeddingGemma/E5/Nomic 等非对称检索模型） | ①`SettingsContext.tsx` `SmartRoutingConfig` 加 `embeddingQueryPrefix`/`embeddingDocumentPrefix`（类型 + 默认 `''` + 读取映射 `|| ''`）；②`SettingsPage.tsx` temp state 类型/默认值/初始化各加两字段 + `handleSmartRoutingConfigChange` 键联合类型扩两键 + 两条保存路径（`handleSmartRoutingConfigChange` 批量、`handleSaveSmartRoutingConfig`）各加两字段变更检测写 `updates` + embeddingMaxTokens 与 progressiveDisclosure 之间新增「嵌入任务前缀」输入块（说明文案 + query/document 两 mono 输入框，placeholder `task: search result \| query: ` / `title: none \| text: `，各带 env 遮蔽警示）；③4 个 locales 各 +5 键（`noChanges` 为补齐——桌面代码此前已调用该键、一直走 fallback，origin #1212 顺带补了 locale 键）。 |
| `815c91a` #1211 | fix(css): 按钮 hover 指针光标 | `index.css` 顶部（`@custom-variant` 行后）加 `@layer base { button:not(:disabled), [role='button']:not(:disabled) { cursor: pointer; } }`（与 origin 逐行一致，桌面端该区域无自定义差异）。 |

**已镜像到 desktop（Rust 后端）**：无。逐项评估：

| 来源 commit | 说明 | 处理决策 | 原因分析 |
| ----------- | ---- | -------- | -------- |
| `f6e4cc8` #1210 后端 | `smartRouting.ts` env 变量解析优先级重构（~195 行）+ `serverController.ts` 在 systemConfig 响应中上报 `envOverriddenFields` | **无需镜像** | Smart Routing 未实现（§7 待办）；env 解析/遮蔽检测全部在 origin Node 后端链路，桌面 Rust 无 Smart Routing 运行时，`envOverriddenFields` 恒为 `[]` → 警示 UI 永不触发（与「区块隐藏」一致，仅代码保留对齐）。 |
| `7b2822d` #1212 后端 | `vectorSearchService.ts` embed 前应用 query/document 前缀 + `smartRouting.ts` 配置键校验扩两键 + `serverController.ts` 入参校验 | **无需镜像** | 桌面端无 Smart Routing 向量检索链路（RAG embedding 用本地 gguf，无前缀注入点）；新配置键经 `config_service::update` JSON 深合并透明 round-trip，前端发什么存什么。 |
| `f49ba5b` / `10736a2` #1213 / `85a530f` #1214 | 纯 docs（smart-routing 指南 Accordion 修复、Mintlify MDX CI 校验脚本 + pnpm-lock、agent shell PATH 文档） | **不同步** | docs/ 按同步策略不同步；桌面端有自己的 AGENTS.md 体系；CI 校验脚本属 origin 文档管线。 |

**同步操作**：子模块指针 `f8615ab -> 85a530f`（工作区已在 85a530f，无需 checkout）；版本号 `1.0.40001 -> 1.0.40002`（四源 + Cargo.lock）；changelog `doc/upgrade/1.0.40002.md` 单文件。

**同步后验证**：`cd frontend && npm run build` 通过（1.00s）；`npx tsc --noEmit` 19 错误 = HEAD 基线 19（git worktree 对照逐文件分布一致，零新增；文档历史记录的 24 基线在 openapi 会话提交后已自然漂移至 19）；locales JSON 四文件 `json.load` 校验通过，6 个新键四语言齐全；本轮无 Rust 源码改动，跳过 `cargo check`。

**影响功能点与结果**：

- **影响功能点**：①设置页 Smart Routing 区块（桌面 Tauri 运行时隐藏，仅 web dev 模式可见）：新增「嵌入任务前缀」输入块（query/document 两输入框）+ 5 个 provider 输入框的 env 遮蔽警示（桌面后端不上报 `envOverriddenFields`，警示恒不显示）；②`SettingsContext`/`configService` 新增 3 个配置字段类型（`embeddingQueryPrefix`/`embeddingDocumentPrefix`/`envOverriddenFields`，读取/保存链路）；③web 模式全部按钮 hover 指针光标（桌面端 webview 内同样生效）；④locales settings 段 +6 键 ×4 语言。
- **结果**：桌面端 Smart Routing 功能行为不变（未实现、区块隐藏，隐藏区块代码与 origin 保持对齐）；新配置键可透传存储，为将来实现 Smart Routing 预留；按钮指针光标为纯视觉改善。用户无需任何操作。
- **发布**：版本 `1.0.40002`，changelog `doc/upgrade/1.0.40002.md`。

#### 2026-09-24（第二轮）：同步 `8ed6478` -> `f8615ab`（2 个 commit，跨 v1.0.40 release + 1 个未发布提交）

`cd mcphub-origin && git --no-pager log --oneline 8ed6478..f8615ab` 共 2 个 commit（`984028e` #1205/#1206 = v1.0.40 tag 点、`f8615ab` #1209）；`git diff --stat 8ed6478..f8615ab -- frontend/ locales/` **为空——本轮 origin 无任何 frontend/locales 改动**，桌面端前端零改动。

**已同步到 desktop（前端 / locales）**：无。**已镜像到 desktop（Rust 后端）**：无。逐项评估：

| 来源 commit | 说明 | 处理决策 | 原因分析 |
| ----------- | ---- | -------- | -------- |
| `984028e` #1205/#1206 | fix(db): PostgreSQL 连接池卡死加固——pg 侧 `statement_timeout`/`idle_in_transaction_session_timeout` 安全阀、pg-pool `destroy()` 加 15s 上限防 reconnect 永久卡死、健康检查查询加超时、重连路径跳过 `synchronize()` DDL、Better Auth pg Pool 对齐主池参数、每请求 systemConfig 读取改内存缓存优先 | **无需镜像** | 全部针对 **PostgreSQL/TypeORM/pg-pool** 网络数据库场景。桌面端为 sqlx SQLite 本地文件（无网络断连、无 pg-pool teardown wedge），sqlx 池自带 `acquire_timeout`；桌面端无独立 Better Auth pg 池、无 DB 健康检查/重连循环；Rust 侧 system_config 直接读 SQLite（本地毫秒级），不存在「池退化拖垮热路径」。无可映射落点。 |
| `f8615ab` #1209 | fix(smart-routing): `updateSystemConfig` 白名单补 `serverDescriptionMode` / `azureOpenaiEmbeddingModel` 两个 Smart Routing 配置键持久化 | **无需镜像** | 桌面端 Smart Routing 未实现（§7 待办）；`config_service::update` JSON 深合并对任意键透明 round-trip 持久化，前端发什么存什么，无需 Rust 白名单。 |

**同步操作**：子模块指针 `8ed6478 -> f8615ab`；版本号 `1.0.39001 -> 1.0.40001`（基线跟随 origin 最新 tag v1.0.40，39 → 40，四源 + Cargo.lock）；changelog `upgrade/1.0.40002.md` 单文件。

**同步后验证**：本轮无 frontend/locales/Rust 源码改动（仅版本号四源 + Cargo.lock），跳过 build/cargo check。

**影响功能点与结果**：

- **影响功能点**：无（本轮 origin 纯 Node 后端修复，桌面端无可映射落点）。
- **结果**：桌面端行为与上一版本（1.0.39001）完全一致；版本号仅跟随 origin v1.0.40 tag 递增。用户无需任何操作。
- **发布**：版本 `1.0.40001`，changelog `upgrade/1.0.40002.md`。

#### 2026-09-24（第一轮）：同步 `6b1fdb7` -> `8ed6478`（43 个 commit，跨 v1.0.36 ~ v1.0.39 四个 release）
...
**同步操作**：子模块指针 `6b1fdb7 -> 8ed6478`；版本号 `1.0.35003 -> 1.0.39001`（基线跟随 origin 最新 tag v1.0.39，35 → 39，四源 + Cargo.lock）；changelog `upgrade/1.0.40002.md` 单文件。
...
- **发布**：版本 `1.0.39001`，changelog `upgrade/1.0.40002.md`。
