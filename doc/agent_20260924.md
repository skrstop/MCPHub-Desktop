...
> 设计与执行计划见 `doc/rag_data_source_tree_plan_20260909.md`。三大需求：①导入入口统一为「数据源选择」（文件/文件夹只是选项）；②新增 Git 数据源（gix 纯 Rust 集成、支持认证）；③列表增加树形视图（存量归「文件」节点）。版本 `1.0.35002`（后并入 1.0.35003 发布，1.0.35003 的 changelog 又随 2026-09-24 基线同步并入 `doc/upgrade/1.0.40001.md`，原 `1.0.35003.md` 已删）。
...
- 版本：`1.0.35001 → 1.0.35003`（tauri.conf.json / Cargo.toml / 根 package.json / frontend package.json + Cargo.lock）；changelog **与 1.0.35003 合并为单文件 `doc/upgrade/1.0.35003.md`**（原 `1.0.35002.md` 已删；该文件后续随 2026-09-24 基线同步再并入 `doc/upgrade/1.0.40001.md`，`1.0.35003.md` 已删）。
...
**验证**：`cargo test --lib` **41 passed**；`cargo check` 0 错 0 警；`npm run build` ✓；`npx tsc --noEmit` 24 = 基线。版本 `1.0.35002 → 1.0.35003`（四源 + Cargo.lock）；changelog `doc/upgrade/1.0.35003.md`（后续并入 `doc/upgrade/1.0.40001.md`，原文件已删）。
...
> 下次同步时，使用 `f8615ab` 作为新的基线 SHA 起点（命令：`cd mcphub-origin && git --no-pager log --oneline f8615ab..HEAD`）。
>
> 注：本节「对应 origin tag」指 origin 仓库的最新 tag（与子模块指针所在 commit 未必相同——指针停在 tag 之后的未发布提交上）。本轮基线跟随 origin 最新 tag v1.0.40（39 → 40），版本号 `1.0.39001 → 1.0.40001`，序号从 001 重新开始；本轮 2 个 commit 均为纯 Node 后端修复（无 frontend/locales 改动、无 Rust 镜像落点），changelog `doc/upgrade/1.0.40001.md` 单文件。上一轮（同日第一轮）跨 origin 四个 release（`v1.0.36`~`v1.0.39`，`6b1fdb7 → 8ed6478`，43 个 commit），前端改动集中在 #1175（分组可见性）、#1129（per-user credentials）、#1186（包版本展示）、#1190/#1191/#1193（服务器复制 + 客户端配置预设复制）、#1199（Smart Routing 索引面板），版本号 `1.0.35003 → 1.0.39001`。
...
**同步操作**：子模块指针 `8ed6478 -> f8615ab`；版本号 `1.0.39001 -> 1.0.40001`（基线跟随 origin 最新 tag v1.0.40，39 → 40，四源 + Cargo.lock）；changelog `doc/upgrade/1.0.40001.md` 单文件。
...
- **发布**：版本 `1.0.40001`，changelog `doc/upgrade/1.0.40001.md`。
...
**同步操作**：子模块指针 `6b1fdb7 -> 8ed6478`；版本号 `1.0.35003 -> 1.0.39001`（基线跟随 origin 最新 tag v1.0.39，35 → 39，四源 + Cargo.lock）；changelog `doc/upgrade/1.0.40001.md` 单文件。
...
- **发布**：版本 `1.0.39001`，changelog `doc/upgrade/1.0.40001.md`。
