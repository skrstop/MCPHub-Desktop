...
- `runtimes/skills/install.json`(56 agent,name→home 相对路径)经 `include_str!("../../runtimes/skills/install.json")` **编译进二进制**(打包进程序)。`default_agents()`(pub(crate),单一数据源)解析为 `Vec<SkillAgent>`:id=`slugify(name)`、skillsPath=`~/`+相对路径、碰撞 `-N` 去重(BTreeMap 保证稳定)。
